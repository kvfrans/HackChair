  ...
  success: function(json) {
    ...
    var options = {
      ...
      start_meet: function(event) {
        alert("session key: " + event.session_key + " session id: " +
          event.session_id + " binder id: " + event.binder_id);
      },
      error: function(event) {
        alert("error code: " + event.error_code + " message: " +
          event.error_message);
      },
  ...
  if (snapshot.val() != null) {
    ...
    if (snapshot.val() > 8) {
      swal({
        ...
        $.ajax({
          type: 'GET',
          url: 'http://api.target.com/items/v3/' + urlthing +
            '?key=1Kfdqvy6wHmvJ4LDyAVOl7saCBoKHcSb&id_type=dpci&fields=descriptions,brand',
          crossDomain: true,
          async: false,
          ...
          success: function(data) {
            ...
            swal({
              title: "<a href='" + item.data_page_link + "'>" +
                item.general_description + "</a>",
              text: "by " + item.online_brand_name,
              html: true
            });
          }
        });
